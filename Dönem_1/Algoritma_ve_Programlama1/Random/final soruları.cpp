...
printf("%f",cevrebul(10,10,13,14,10,14));
	//test etmek için 3-4-5 üçgeni kullandim 
...
int ebi,ebj;
int eki,ekj;
int eb=-2147483646;//iki degerede alabilecegi en küçük degeri verdim en büyük sayy en az bu olabilir :D:D

//kullanycynyn girdigi sayi bu sayidan büyük yada esit olmak zorunda

int ek=2147483646;//int sinirlarinda bu sayidan daha büyük bi sayy yok 
//kullanycy ne girerse girsin bu sayyya e?t yada küçük girmek zorunda
for(int i=0;i<=1;i++)
	{
for(int j=0;j<=1;j++){
...
int sayac=0;
for(int i=0;i<5;i++)// ilk girilen kelimeyi indis indis dola?mak için bir for actyk 

{
for(int j=0;j<5;j++)//ilk kelimenin içinde bu indisteki harften kaç tane var bulduk
...
for(int k=0;k<5;k++)//metin2 içinde bu harften kaç tane var ona baktyk{
...
if(sayac!=0)//eger sayac syfyrsa ilk girilen metin içerisindeki i. indisteki harf iki kelimede de e?it sayyda bulunmaktadyr
{
